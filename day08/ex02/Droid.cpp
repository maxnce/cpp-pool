...
Droid &Droid::operator=(const Droid &other)
{
    ...
    this->BattleData = (new DroidMemory(*other.getBattleData()));
    return *this;
}
...
bool Droid::operator()(const std::string *task, size_t xp)
{
    ...
    this->Energy -= 10;
    this->setStatus(new std::string(*task));
    if (this->getBattleData()->getExp() >= xp) {
        *this->Status += " - Completed!";
        this->getBattleData()->getExp() += (xp / 2);
        return true;
    }
    *this->Status += " - Failed!";
    this->getBattleData()->getExp() += xp;
    return false;
}
