...
Hostname::Hostname() : IMonitorModule(HOSTNAME)
{
...
void Hostname::display(int &x, int &y, SystemProperties sys)
{
    if (("Host: " + sys.getHost()).length() + 5 > 30 || ("Name: " + sys.getUser()).length() + 5 > 30) {
        this->_ncursesWidth = std::max(("Host: " + sys.getHost()).length(), ("Name: " + sys.getUser()).length()) + 5;
    } 
    ...
    attron(COLOR_PAIR(1));
    mvprintw(y * 5, x - this->_ncursesWidth + 2, "> Hostname");
    attroff(COLOR_PAIR(1));
    attron(COLOR_PAIR(this->getSelect() ? 4 : 3));
    mvprintw(y * 5, x - 2, "X");
    attroff(COLOR_PAIR(this->getSelect() ? 4 : 3));
    mvprintw(y * 5 + 2, x - this->_ncursesWidth + 6, ("Host: " + sys.getHost()).c_str());
    mvprintw(y * 5 + 3, x - this->_ncursesWidth + 6, ("Name: " + sys.getUser()).c_str());
    wrefresh(tempBox);
}

void Hostname::display(int &x, int &y, SystemProperties sys, sf::RenderWindow *win, sf::Texture *text)
{
